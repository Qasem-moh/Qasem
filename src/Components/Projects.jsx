import {Typography} from "@material-tailwind/react";

function Projects() {
    return (
        <div className="w-full mt-40">
            <hr/>
            <h1 className="ml-8 mt-4">Projects</h1>
            <div className="relative right-0">
                <div data-tab-content="" className="p-5">
                    <div className="block opacity-100" id="app" role="tabpanel">
                        <div role="tabpanel"
                             className="grid w-full grid-cols-3 gap-4 p-4 font-sans text-base antialiased font-light leading-relaxed text-gray-700 h-max"
                             data-value="html">
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/tcqdrNx/1.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                Store Book
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">
                                                React,MongoDB,NodeJS,Tailwind
                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://crudbooks.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/6vn1zhQ/2.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                Togo20
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">

                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://togo20.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/L6n6h1V/3.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                Store Drinks
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">

                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://drinks2.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/NW10Vnb/4.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                Virtual Store
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">

                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://virtualstored.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/jTtLC6M/5.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                HangMan
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">

                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://hangman-man.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                            <div>
                                <figure className="relative h-96 w-full">
                                    <img
                                        className="h-full w-full rounded-xl object-cover object-center"
                                        src="https://i.ibb.co/VwHPcmp/6.png"
                                        alt="nature image"
                                    />
                                    <figcaption
                                        className="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                        <div>
                                            <Typography variant="h5" color="blue-gray">
                                                Movies Store
                                            </Typography>
                                            <Typography color="gray" className="mt-2 font-normal">

                                            </Typography>
                                        </div>
                                        <Typography variant="h5" color="blue-gray">
                                            <a href="https://main--moviesstore2024.netlify.app/">View</a>
                                        </Typography>
                                    </figcaption>
                                </figure>
                            </div>
                        </div>
                    </div>


                </div>
            </div>
        </div>
    );
}

export default Projects;